 ...
 */
package ru.javarush.taskbook.web.rest.exceptions.handlers;
 ...
 * @author ilyakhromov
 *
 * Now this class is ablel to handle annotated exceptions ang change model correspondingly.
 * works only with @ResponseStatus annotation
 */
public class DefaultRestErrorResolver implements RestErrorResolver, MessageSourceAware, InitializingBean {
    ...
    private static final LoggerWrapper log = LoggerWrapper.get(DefaultRestErrorResolver.class);
    ...
    protected int getStatusValue(RestError template, ServletWebRequest request, Exception ex) {
        if(ex.getClass().getDeclaredAnnotation(ResponseStatus.class) != null){
            int status  = ex.getClass().getDeclaredAnnotation(ResponseStatus.class).value().value();
            if(status != 0) {
                log.warn("Response status was changed to " + status + " due to business annotation priority");
                return status;
            }
        }
    ...
    protected String getMessage(RestError template, ServletWebRequest request, Exception ex) {
        if(ex.getClass().getDeclaredAnnotation(ResponseStatus.class) != null){
            String msg  = ex.getClass().getDeclaredAnnotation(ResponseStatus.class).reason();
            if(msg != null && !msg.isEmpty()) {
                log.warn("Response message was changed to \"" + msg + "\" due to business annotation priority");
                return msg;
            }
        }
